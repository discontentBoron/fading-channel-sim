...
from zx_stat_sum_sin import generate_rayleigh_fading_zx
import jakes_spectrum
...
def simulate_qpsk():
    ...
    for v_kph in speeds:
        t, h_t, fd_max = generate_rayleigh_fading_zx(M, N, f_s, f_c, v_kph)
...
plt.xlabel("Eb/N0 (dB)")
plt.ylabel("BER")
plt.title("BER vs SNR under Rayleigh Fading (Zheng-Xiao model)")
plt.legend()
